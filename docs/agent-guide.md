...
1. Read `.claude/task.md` — and nothing else as a work queue. Check its
   ...
   `For: Coder`, the tier is satisfied, and it contains a numbered
   subtasks list (1 to N), work through that list in the order given —
   that is the "recommended order." Resume the first 🚧 In Progress subtask, otherwise start at the first
   🆕 New. An earlier 👀 Review subtask blocks starting a later one. Complete one subtask, then stop and hand control back for
...
`docs/agents/`. Those files are authoritative for what each role may
...
1. **Coder** — asked to work `.claude/task.md`, which is the only file it
   ...
   for investigating — use `bash` instead. Use the host's available file tools. Prefer Read/Grep/Glob and
   Edit/Write in Claude; in Codex use `rg`, scoped reads and `apply_patch`.
   Run project scripts through Bash. Do not install heavy tools for a
   routine investigation. If a problem surfaces that
...
3. **Task writer** — asked to turn a user request, the open entries in
   ...
   keeping every subtask in a task assigned to its single `For:` role. Decompose a large task or
...
**Effort** — story points, Fibonacci, judged as work for the assigned
owner including tests and documentation updates:
...
- `8` — the answer is not known before the work starts: a design question
  whose options are not enumerated yet, a boundary that has to move, or
  a refactor across several crates with no established pattern to follow.
...
- Order Plan: a numbered list (1 to N) giving the subtask order, each
  ...
  subtask consumes. No value may require more than the task's
  `Model tier:`; a stronger session may work the task. Problem-repair
  tasks batch one band, while a roadmap task may contain lower bands and
  takes the tier of its highest-rated subtask.
...
The canonical list remains in `CLAUDE.md` because `scripts/check_docs.sh`
reads that section directly. Read it before claiming a capability works.
...
No cycles, no back-edges, and no edge beyond this list. Cargo rejects
normal-dependency cycles; the specific allowed-edge list is a project
policy, not something the compiler enforces — see `docs/adr/0002-crate-splitting.md` for why.
...
lines — finishes with
`grep -rn "<module>.rs:" docs docs/agent-guide.md` and re-points every hit against
...
milestone runs `grep -rn "\.rs:[0-9]" docs docs/agent-guide.md` over the whole
...
severity, and whether a client should retry) independently of the server or client protocol. See `crates/common/src/error.MD` and
...
or `#[tracing::instrument]`, never constructs a subscriber. Each binary initializes its own subscriber; see `crates/cli/src/main.MD`
and `crates/server/src/main.MD` for configuration. `cli` writes to **stderr**, not stdout — see
`crates/cli/src/main.MD` for why: stdout there is an existing, tested REPL
contract, not the general rule's target. The headless server binary
writes JSON to stdout, exactly as the rule says, since it has no
...
Claude branches use `<type>/<kebab-description>`. Codex branches use
`codex/<kebab-description>` unless the human specifies another name.
