...
(`crates/storage/src/buffer.rs:750-751`) forces the log durable to the
...
(`crates/txn/src/manager.rs:104-105`), and `storage::recovery::recover`
...
exclusive lock per inserted `Rid` (line 73), and `CREATE INDEX` takes an
...
lock early: `TransactionManager::commit` and the abort's `finish_abort`
call `LockManager::release_all` as their last step
(`crates/txn/src/manager.rs:111,138`). That is strict two-phase locking, so
...
(`crates/txn/src/manager.rs:42,59,72-73,92,106`).
...
`ReadCommitted` for the one-page transaction a checkpoint opens to stamp
its LSN into the page-0 header (`crates/txn/src/checkpoint.rs:34`) — the
checkpoint's own `CheckpointBegin`/`CheckpointEnd` records carry the
reserved `CHECKPOINT_TXN` id instead and belong to no transaction at all
(`crates/txn/src/checkpoint.rs:27,31`). `ReadUncommitted`,
`RepeatableRead` and `Serializable` are names with no behaviour behind
them.
